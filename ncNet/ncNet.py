...
class ncNet(object):
    ...
    def specify_dataset(self, data_type, db_url = None, table_name = None, data = None, data_url = None):
        ...
        if data_type == 'csv':
            if data is not None and data_url == None:
                self.data = data
            ...
            if data is not None and type(data) == pd.core.frame.DataFrame:
                self.data = data
            else:
        ...
        if data_type == 'csv':
            table_cols[table_name] = self.datadf.columns.values
        print(table_cols)
    ...
    def nl2vis(self, nl_question, chart_template=None, show_progress=None, visualization_aware_translation=True):
        ...
        if visualization_aware_translation == True:
            ...
            # print('[The Predicted VIS Result]:')
            print(query2vl.to_VegaLite(pred_query, self.data))
            return VegaLite(query2vl.to_VegaLite(pred_query, self.data)), query2vl.to_VegaLite(pred_query, self.data)
    ...
    def save_json_vega_specs(self, specs, filename):
        file_name = filename
        with open(file_name, 'w') as json_file:
            json.dump(specs, json_file, indent=2)
